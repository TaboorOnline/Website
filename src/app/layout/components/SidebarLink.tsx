    ...
    <Link
      ...
      className={`flex items-center py-2.5 text-sm rounded-md transition-all duration-200 group relative ${
        ...
        isActive
          ...
          : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/70 hover:text-indigo-600 dark:hover:text-indigo-400'
      }`}
      title={!isSidebarOpen ? label : undefined} // Tooltip when collapsed
    >
      {/* Active indicator bar */}
      {isActive && (
        <span className={`absolute ${isSidebarOpen ? 'left-0 top-0 bottom-0 w-1 my-1' : 'left-0 top-1/2 -translate-y-1/2 h-6 w-1'} bg-indigo-500 dark:bg-indigo-400 rounded-r-full transition-all duration-200`}></span>
      )}
      
      {/* Icon with animation on hover */}
      <span className={`flex-shrink-0 text-lg transition-transform duration-200 ${isSidebarOpen ? 'ml-2 mr-3' : 'mx-auto'} ${isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400'} group-hover:scale-110`}>
        {icon}
      </span>
